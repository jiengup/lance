...
pub use lance_table::format::RowIdRange;
...
impl Dataset {
    ...
    /// `None`.
    pub async fn reserved_row_ids(&self) -> Result<Option<RowIdRange>> {
    ...
    /// Returns the stable row-id range generated by the current dataset
    /// version, if any.
    ///
    /// This only reports row ids generated by the current version's write
    /// transaction. It does not scan older versions or summarize history.
    ///
    /// If the current version was created by `ReserveRowIds` then this returns
    /// `None`, because reserving row ids does not write any rows.
    pub async fn generated_row_ids(&self) -> Result<Option<RowIdRange>> {
        if !self.manifest.uses_stable_row_ids() {
            return Err(Error::invalid_input(
                "generated row ids require a dataset created with stable row ids".to_string(),
            ));
        }
        ...
        let rows_written = match &transaction.operation {
            Operation::ReserveRowIds { .. } => return Ok(None),
            Operation::Append { fragments, .. } | Operation::Overwrite { fragments, .. } => {
                Self::count_fragment_rows(fragments.as_slice())?
            }
            _ => return Ok(None),
        };

        let row_id_range = match &transaction.operation {
            Operation::Append {
                row_ids: Some(row_id_range),
                ..
            } => RowIdRange {
                start_row_id: row_id_range.start_row_id,
                num_rows: rows_written,
            },
            Operation::Append { row_ids: None, .. } | Operation::Overwrite { .. } => {
                let start_row_id = self
                    .manifest
                    .next_row_id
                    .checked_sub(rows_written)
                    .ok_or_else(|| {
                        Error::internal(format!(
                            "Manifest next_row_id={} is smaller than generated num_rows={}",
                            self.manifest.next_row_id, rows_written
                        ))
                    })?;
                RowIdRange {
                    start_row_id,
        ...
        Ok(Some(row_id_range))
    }

    fn count_fragment_rows(fragments: &[lance_table::format::Fragment]) -> Result<u64> {
        fragments.iter().try_fold(0_u64, |total_rows, fragment| {
            let fragment_rows = fragment
                .physical_rows
                .ok_or_else(|| Error::internal("Fragment does not have physical rows"))?
                as u64;
            total_rows.checked_add(fragment_rows).ok_or_else(|| {
                Error::invalid_input(format!(
                    "row count overflow when summing fragment rows: total_rows={}, fragment_rows={}",
