...
async fn build_dataset(shape: Shape) -> (TempStrDir, Arc<Dataset>) {
    ...
    match shape {
        Shape::Clean => {}
        Shape::WithNewRowsOnly => {
            ds = append_rows(&path, base, 500).await;
        }
        Shape::WithDeletionsOnly => {
...
fn bench_one_shape(c: &mut Criterion, name: &str, shape_factory: impl Fn() -> Shape) {
    let rt = tokio::runtime::Runtime::new().unwrap();
    let (_dir, ds) = rt.block_on(build_dataset(shape_factory()));
...
fn bench_merge_insert(c: &mut Criterion) {
    bench_one_shape(c, "merge_insert/clean", || Shape::Clean);
    bench_one_shape(c, "merge_insert/with_new_rows_only", || {
        Shape::WithNewRowsOnly
    });
    bench_one_shape(c, "merge_insert/with_deletions_only", || {
        Shape::WithDeletionsOnly
    });
    // The shape that exercises the AllowList(Full) - BlockList(Partial) path.
    bench_one_shape(c, "merge_insert/with_new_rows_and_deletions", || {
        Shape::WithBoth
    });
}
